...
clear; clc; close all;
% Ensure all helper functions are on the MATLAB path
...
fprintf('--- Testing find_transformation.m ---\n');
...
% 2. A point closest to a vertex.
% 3. A point closest to an edge.
fprintf('--- Testing closest_point_on_triangle.m ---\n');
% 1. Define a simple triangle in the XY plane
v1_tri = [0; 0; 0];
v2_tri = [10; 0; 0];
v3_tri = [0; 10; 0];
% 2. Test 1: Point inside the triangle
p_inside = [2; 2; 0];
...
% 3. Test 2: Point closest to a vertex (v1)
p_vertex = [-5; -5; 0];
...
% 4. Test 3: Point closest to an edge (v1-v2)
p_edge = [5; -5; 0];
...
% Test: Create a simple mesh with two triangles far apart.
% A query point near T1 should return a point on T1.
% A query point near T2 should return a point on T2.
% This proves the function iterates all triangles for a global minimum.
fprintf('--- Testing closest_point_on_mesh.m ---\n');

% 1. Define Vertices for a 2-triangle mesh
...
% 2. Define Triangles (1-based indices)
% Format: [idx1, idx2, idx3, n1, n2, n3] (neighbors are -1)
triangles = [ 1, 2, 3, -1, -1, -1;
              4, 5, 6, -1, -1, -1 ];
          
% 3. Test Case 1: Point is on T1
p1_mesh = [2; 2; 0]; % Point is on T1
...
% 4. Test Case 2: Point is on T2
p2_mesh = [101; 0; 1]; % Point is on T2
